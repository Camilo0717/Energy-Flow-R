...
const Bezier = L.Path.extend({
  ...
  setAnimatePlane(path) {
    ...
    const spaceship_img = (this.spaceship_img = SnapSvg.image(
      this.icon.path
    ).attr({
      visibility: "hidden"
    }))
    ...
    if (
      this.options.iconTravelLength &&
      !isNaN(parseFloat(this.options.iconTravelLength))
    ) {
      destinationPathLength =
        fullPathLength * parseFloat(this.options.iconTravelLength)
    }

    const halfPathLength =
      destinationPathLength -
      destinationPathLength /
        (this.options.easeOutPiece ? this.options.easeOutPiece : 50)
    ...
    width = Math.min(
      Math.max(width, 30),
      this.options.iconMaxWidth ? this.options.iconMaxWidth : 50
    )
    height = Math.min(
      Math.max(height, 30),
      this.options.iconMaxHeight ? this.options.iconMaxHeight : 50
    )

    const fullAnimatedTime = this.options.fullAnimatedTime
      ? this.options.fullAnimatedTime
      : 7000
    const easeOutTime = this.options.easeOutTime
      ? this.options.easeOutTime
      : 2500
    ...
    Snap.animate(
      ...
      step => {
        ...
        spaceship.transform(
          `translate(${x},${y}) rotate(${moveToPoint.alpha - 90}, ${width /
            2}, ${height / 2})`
        )
      },
      easeOutTime,
  ...
  getMidPoint(from, to, deep, round_side = "LEFT_ROUND") {
    let offset = 3.14
